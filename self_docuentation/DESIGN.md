...
The system uses a modular, component-based approach for managing system prompts and context injections. This design eliminates scattered prompt strings and provides a clean, maintainable way to assemble prompts programmatically.

**Architecture**:

```
backend/zikos/services/prompt/
...
1. **PromptSection Base Class**: Abstract base with `render()` and `should_include()` methods
2. **SystemPromptBuilder**: Composes sections in order, filters by `should_include()`, builds final prompt
3. **Core Sections**:
   - `CorePromptSection`: Loads core prompt from SYSTEM_PROMPT.md file
   - `MusicFlamingoSection`: Conditionally includes Music Flamingo instructions (if service configured)
   - `ToolInstructionsSection`: Dynamically generates tool documentation at runtime
4. **Audio Context Formatters**: Helper classes for dynamic message injection:
   - `AudioAnalysisReminderSection`: Short reminder for context injection
   - `AudioAnalysisFeedbackReminderSection`: Detailed reminder for feedback generation
   - `AudioAnalysisContextFormatter`: Static methods for formatting analysis context messages

**Benefits**:
- **Modular**: Each section is self-contained and testable independently
- **Composable**: Easy to add/remove/reorder sections
- **Type-safe**: Clear interfaces and inheritance
- **Maintainable**: Changes are localized to specific sections
- **No scattered strings**: All prompt content is in the prompt module
...
# Formatting audio analysis context
message = AudioAnalysisContextFormatter.format_analysis_context(
    analysis_data, user_question
)
```

**Design Principles**:
- All prompt content lives in the `prompt` module, never scattered in service code
- Sections can be conditional (e.g., Music Flamingo only if configured)
- Dynamic content (tools, analysis) is injected via specialized sections/formatters
- Easy to test individual sections in isolation
